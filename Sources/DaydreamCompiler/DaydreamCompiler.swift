...
public class DaydreamCompiler
{
    ...
    public func compile(wordSize: WordSize) throws
    {
        ...
        guard sorted.count < wordSize.max else
        {
            throw DaydreamCompilerError.tooManyTypes
        }
        ...
        try self.compile(builtins, identifiers, namespace, wordSize, outputURL)
    }

    func compile(_ builtins: [Identifier], _ identifiers: [Identifier], _ namespace: Namespace, _ wordSize: WordSize, _ outputDirectory: URL) throws
    {
        try self.writeTypeIdentifiers(builtins, identifiers, namespace, wordSize, outputDirectory)
    }

    func writeTypeIdentifiers(_ builtins: [Identifier], _ identifiers: [Identifier], _ namespace: Namespace, _ wordSize: WordSize, _ outputDirectory: URL) throws
    {
        ...
        public enum TypeIdentifiers: \(wordSize.typeName)
        {
            ...
            public init?(varint: Data)
            {
                ...
                let uint64 = UInt64(bint)
                self.init(rawValue: uint64)
            }
                    ...
                    \(cases.map
                      {
                        ...
                        public init?(data: Data)
                        {
                            ...
                            let uint64 = UInt64(int)

                            guard let type = TypeIdentifiers(rawValue: uint64) else
                            {
                                return nil
                            }
                ...
                if identifier.name == "Varint"
                {
                    return """
                                case .Varint(let bignum):
                                    return (TypeIdentifiers.VarintType.varint + bignum.varint).pushLength()
                    """
                }
                    ...
                    case .SingletonType(name: _):
                        return """
                                    case .\(identifier.name):
                                        return TypeIdentifiers.\(identifier.name)Type.varint
                        """

                    case .Record(name: _, fields: _):
                        return """
                                    case .\(identifier.name)(let subtype):
                                        return (TypeIdentifiers.\(identifier.name)Type.varint + subtype.data)
                        """
